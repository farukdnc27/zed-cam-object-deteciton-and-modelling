...
class StoneDimensionEstimator:
    def __init__(self,
        ...
        self.scale_factor = 1.0  # F ile düzeltilecek
    ...
    # ---------- OBB-local voxelization & packing ----------
    @staticmethod
    def _mesh_transform(mesh, R, t):
        """ mesh' e 4x4 [R|t] uygular (R 3x3, t 3,) """
        T = np.eye(4)
        T[:3, :3] = R
        T[:3, 3] = t
        mesh2 = o3d.geometry.TriangleMesh(mesh)  # kopya
        mesh2.transform(T)
        return mesh2
    ...
    def _project_3d_to_pixel(self, p3):
        # ZED koordinatları zaten kamera uzayında; intrinsics ile projekte et
    ...
    def run_customer_demo(self):
        ...
        should_exit = {"v": False}
        placed_boxes_world = []  # [(minx,miny,minz, sx,sy,sz, center3d) mm]
        ...
        try:
            while not should_exit["v"]:
                ...
                if k == ord('q'):
                    ...
                    self.calib_clicks.clear()
                    print("Kalibrasyon: OpenCV penceresinde 2 noktaya tıklayın, sonra konsola gerçek mm girin.")
                    ...
                    if placed_boxes_world:
                        ...
                        try:
                            with open(fname, "w", encoding="utf-8") as f:
                                ...
                                for (vx, vy, vz, sx, sy, sz, cc) in placed_boxes_world:
                ...
                try:
                    ...
                    if stone_mesh is not None and measurements is not None:
                        ...
                        try:
                            ...
                            # Kutuları lokalda oluştur, sonra dünyaya geri taşı (R, c)
                            for idx, (ix, iy, iz, sx, sy, sz) in enumerate(placed, start=1):
                                lx = origin_local[0] + ix * self.voxel_size_mm
                                ly = origin_local[1] + iy * self.voxel_size_mm
                                lz = origin_local[2] + iz * self.voxel_size_mm
                                wx = sx * self.voxel_size_mm
                                wy = sy * self.voxel_size_mm
                                wz = sz * self.voxel_size_mm

                                # Kutu lokal uzayda min köşeye yerleşecek
                                ...
                                vis.add_geometry(box_world)

                                # Dünya koordinatlarını ve merkezini kaydet (CSV & etiket)
                                min_world = (R @ np.array([lx, ly, lz])) + c
                                center_local = np.array([lx + wx/2.0, ly + wy/2.0, lz + wz/2.0])
                                center_world = (R @ center_local) + c
                                placed_boxes_world.append((min_world[0], min_world[1], min_world[2], wx, wy, wz, center_world))
                    ...
                    # kalibrasyon tıklarını işaretle
                    for (cx, cy) in self.calib_clicks:
                        cv2.circle(image_bgr, (cx, cy), 6, (0, 0, 255), -1)

                    # Kutu indekslerini görüntüye yaz (merkezleri projekte ederek)
                    for idx, (_, _, _, _, _, _, center3d) in enumerate(placed_boxes_world, start=1):
                        pt = self._project_3d_to_pixel(center3d)
                        if pt is not None:
                            u, v = pt
                            if 0 <= u < w and 0 <= v < h:
                                cv2.circle(image_bgr, (u, v), 4, (0, 0, 255), -1)
                                cv2.putText(image_bgr, f"{idx}", (u+6, v-6),

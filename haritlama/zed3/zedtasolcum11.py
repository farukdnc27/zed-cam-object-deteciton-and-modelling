...
class StoneDimensionEstimator:
    ...
    def voxelize_mesh_to_bool(self, mesh, voxel_size_mm):
        ...
        if _HAS_TRIMESH:
            tm = trimesh.Trimesh(vertices=np.asarray(mesh.vertices),
                                 ...
                                 process=False)
            # Ray casting yerine direkt "contains" ile dolduruyoruz
            ...
            # Basit ve daha kaba bir yaklaşım: yüzeyden örnekleme ile
    ...
    # ---------- Surface-aware support map & red prism ----------
    def build_surface_support(self, grid):
        """
        grid: (nz,ny,nx) True=taşın içinde (OBB-lokal)
        Dönüş:
          top_idx: (ny,nx) her kolon için en üstteki True voxel'ın z indexi (yoksa -1)
          red_prism_boxes: Open3D mesh listesi (yüzey zarfını göstermek için 1-voxel sütunlar)
        """
        nz, ny, nx = grid.shape
        top_idx = np.full((ny, nx), -1, dtype=int)

        # Üstten aşağı tara (z büyük -> küçük)
        for iy in range(ny):
            col = grid[:, iy, :]  # (nz, nx)
            # her x için ilk True olan z (üstten)
            # üstten bakmak için ters çevir
            rev = col[::-1, :]  # (nz, nx)
            first_true = np.argmax(rev, axis=0)  # eğer hiç True yoksa 0 döner ama kontrol edeceğiz
            has_true = np.any(rev, axis=0)
            # gerçek z indexine çevir: iz = nz-1 - first_true
            ti = np.where(has_true, (nz - 1 - first_true), -1)
            top_idx[iy, :] = ti

        # Görselleştirme için "kırmızı yüzey prizması": her (iy,ix) için 1 voxel kalınlıkta sütun
        red_meshes = []
        if np.any(top_idx >= 0):
            # voxel -> world dönüşümünü dışarıda yapacağız; şimdilik lokal koordinatta üret
            # Bunun için OBB-lokal min bound ve voxel_size'a ihtiyaç var -> dışarıda üretelim
            pass  # sadece indeksleri döneceğiz; mesh üretimi dışarıda, OBB dönüşümü ile yapılacak
        return top_idx

    # ---------- Support-aware packing ----------
    def greedy_pack_support_aware(self, grid, piece_vox_shape, top_idx):
        """
        grid: (nz,ny,nx) True=taşın içinde
        piece_vox_shape: (pz,py,px) voxel cinsinden
        top_idx: (ny,nx) her kolon için yüzey üst voxel z indexi
        Strateji:
          - Kutuyu yüzeye "oturt" (kutunun ÜSTÜ yüzeyde olacak şekilde), yani z_top = min(top_idx footprint)
          - Kutunun yüksekliği pz kadar AŞAĞI doğru uzasın: [z_top-pz+1, z_top] aralığı dolu olmalı.
          - Tüm footprint boyunca bu şarta uyan ilk lokasyona yerleştir; sonra alanı doldur.
        """
        nz, ny, nx = grid.shape
        ...
        for (pz_, py_, px_) in rots:
            for iy in range(ny - py_ + 1):
                for ix in range(nx - px_ + 1):
                    # Bu footprint'teki yüzey seviyeleri:
                    local_top = top_idx[iy:iy + py_, ix:ix + px_]
                    if np.any(local_top < 0):
                        continue  # bazı kolon tamamen boş

                    # Kutunun üstünü yüzeye oturtmak için ortak üst z:
                    # Ayak izi içinde en küçük (yani en derin) top değeri üst sınır olur
                    z_top = np.min(local_top)
                    z_bot = z_top - pz_ + 1
                    if z_bot < 0:
                        continue

                    # Bu hacim grid içinde dolu mu ve daha önce kullanılmamış mı?
                    sub_grid = grid[z_bot:z_top + 1, iy:iy + py_, ix:ix + px_]
                    if sub_grid.shape != (pz_, py_, px_):
                        continue
                    region_ok = np.all(sub_grid)
                    region_free = not np.any(filled[z_bot:z_top + 1, iy:iy + py_, ix:ix + px_])
                    if region_ok and region_free:
                        filled[z_bot:z_top + 1, iy:iy + py_, ix:ix + px_] = True
                        placed.append((ix, iy, z_bot, px_, py_, pz_))  # not: z_bot taban
    ...
    def _perform_full_analysis(self, points_np):
        ...
        if stone_mesh is not None and measurements is not None:
            try:
                ...
                # Yüzey destek haritası (top index)
                top_idx = self.build_surface_support(grid)

                # Kırmızı yüzey prizması (sadece yüzey sütunu, 1-voxel kalınlık)
                red_meshes = []
                ...
                print("5/5 - Destek-bilinçli kutu yerleştirme...")
                placed, used_vox, _ = self.greedy_pack_support_aware(grid, (pz, py, px), top_idx)
                ...
                # Verim, grid içindeki gerçek doldurulan voxel oranı
                total_inside = max(total_inside, 1)
                results["efficiency"] = (used_vox / total_inside) * 100.0
            ...
            while not should_exit["v"]:
                ...
                # ANALIZ
                    ...
                    if self.analysis_results is None:
                        ...
                        # Kırmızı yüzey prizması
                        for rm in self.analysis_results['red_prism_meshes']:
                            vis.add_geometry(rm)
                        # Kutu meshleri
                    ...
                    cv2.putText(image_display, "Mod: Destek-bilincli yuzeyden kesim (kirmizi zarfa gore)",
                    ...
                    cv2.putText(image_display, "Sifirlamak icin 'R' | CSV icin 'E' | Kalibrasyon 'F'",
                ...
                if k == ord('q'):
                    ...
                    # points_np ve image_bgr son frame'den
                    points_np, image_bgr = self.get_point_cloud_and_image()
